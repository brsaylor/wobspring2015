...
public class DCarnivoreAttack : MonoBehaviour {
	...
	void OnTriggerEnter (Collider other)
	{
		...
		if (!inRange) {
			foreach (GameObject Dcarnivore in DcarnivoreList) {
				if (inRange)
					break;
				if (Dcarnivore == other.gameObject && Dcarnivore != this.gameObject) {
					// ... the player is in range.
					inRange = true;
					currentlyAttacking = other.gameObject;
					currentEnemyHealth = currentlyAttacking.GetComponent <Health> ();
			...
			foreach (GameObject Domnivore in DomnivoreList) {
				if (inRange)
					break;
				if (Domnivore == other.gameObject && Domnivore != this.gameObject) {
					// ... the player is in range.
					inRange = true;
					currentlyAttacking = other.gameObject;
					currentEnemyHealth = currentlyAttacking.GetComponent <Health> ();
